...
{
    clearScr();
    cout << space((columns - 50) / 2) << style::blink << fg::magenta << style::bold<< ":)  " << style::reset << fg::magenta << style::bold << "Thanks for visiting us, do come back later" << style::reset << style::blink <<  fg::magenta << style::bold<<" :)" << style::reset << endl;
    showCursor();
...
void greet()
{
    ...
    char *today;
    today = date();
    cout << space((columns - 67) / 2 + 23) << style::blink << "   " << style::italic << rang::fgB::blue << style::bold << today << style::reset << style::italic << style::blink << rang::fgB::blue << " Exclusive Offers! "
         << "  ️\n\n"
         << style::reset;
    cout << space((columns - 67) / 2 + 15) << style::italic << rang::fgB::blue << "Enjoy discounts on all dishes every " << style::bold << today << style::reset << style::italic << rang::fgB::blue << "!" << style::reset << "\n\n";
}
...
map<int, bill> letsDel(map<int, bill> billmap, int lim)
{
    ...
    while (true)
    {
        ...
        if (choice == 1)
        {
            ...
            {
                clearScr();
                cout << space((columns-27)/2)<<style::bold << "All Items Removed from cart" << style::reset << endl;
                cout << space((columns - 50) / 2) << style::blink << fg::magenta << style::bold<< ":)  " << style::reset << fg::magenta << style::bold << "Thanks for visiting us, do come back later" << style::reset << style::blink <<  fg::magenta << style::bold<<" :)" << style::reset << endl;
                showCursor();
                exit(0);
            }
...
void thankYou()
{
    ...
    cout << space((columns - 87) / 2) << "   ██║    ██╔══██║ ██╔══██║ ██║╚██╗██║ ██╔═██╗       ╚██╔╝   ██║   ██║ ██║   ██║    ╚═╝" << endl;
    cout << space((columns - 87) / 2) << "   ██║    ██║  ██║ ██║  ██║ ██║ ╚████║ ██║  ██╗       ██║    ╚██████╔╝ ╚██████╔╝    ██╗" << endl;
    cout << space((columns - 87) / 2) << "   ╚═╝    ╚═╝  ╚═╝ ╚═╝  ╚═╝ ╚═╝  ╚═══╝ ╚═╝  ╚═╝       ╚═╝     ╚═════╝   ╚═════╝     ╚═╝" << style::reset;
}
void setup() // calculate console size for alignment purposes
{
